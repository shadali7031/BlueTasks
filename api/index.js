...
app.get("/api/health", (req, res) =>
  res.json({ ok: true, app: "BlueTasks" })
);
...
app.get("/api/me", async (req, res) => {
  try {
    ...
    const { data: claims, error: claimError } =
      await supabase
        .from("task_claims")
        .select("task_id, reward, claimed_at")
    ...
    const { data: withdrawals, error: wdError } =
      await supabase
        .from("withdrawals")
        .select("*")
        .eq("telegram_id", user.telegram_id)
    ...
    res.json({
      ...
      referralLink:
        `https://t.me/${process.env.BOT_USERNAME || "YOUR_BOT"}` +
        `/?startapp=ref_${user.telegram_id}`
    });
  } catch (e) {
    res.status(401).json({
      ok: false,
...
/*
  ...
  The browser calls this endpoint only after AdsGram's Reward
  promise resolves. The database RPC remains authoritative for:
  - task-specific daily limits
  - reward amount
  - balance update
  - 240 BLC daily cap trigger
  - duplicate/concurrent claim protection

  Important:
  AdsGram's normal Reward SDK callback is a client-side event.
  Do not treat this endpoint alone as cryptographic proof of a
  paid ad event. For higher-value production rewards, configure
  AdsGram's supported Reward URL/server confirmation flow and
  reconcile credits server-side.
*/
app.post("/api/task/complete", async (req, res) => {
  try {
    ...
    const adProvider = String(req.body.adProvider || "");
...
app.post("/api/withdraw", async (req, res) => {
  try {
    ...
    if (!Number.isInteger(amount) || amount < 1500) {
      return res.status(400).json({
        ok: false,
        error: "Minimum withdrawal is 1500 BLC"
      });
    }
...
if (require.main === module) {
  ...
  app.listen(port, () =>
    console.log(`BlueTasks running on ${port}`)
  );
}
